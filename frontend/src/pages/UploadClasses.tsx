...
import type { ClassBulkUploadResult } from '../types';

export const UploadClasses: React.FC = () => {
    const [file, setFile] = useState<File | null>(null);
    const [isDownloading, setIsDownloading] = useState(false);
    const [isUploading, setIsUploading] = useState(false);
    const [result, setResult] = useState<ClassBulkUploadResult | null>(null);

    const handleDownloadTemplate = async () => {
        setIsDownloading(true);
        try {
            const blob = await adminApi.downloadClassesTemplate();
    ...
    const handleUpload = async () => {
        ...
        setIsUploading(true);
        try {
            const data = await adminApi.uploadClassesCsv(file);
            setResult(data);
    ...
    return (
        <div className="space-y-6 max-w-4xl">
            <div>
                <h1 className="text-2xl font-bold text-white">Upload Classes</h1>
                <p className="text-white/60 text-sm">Admin bulk upload for class mapping (department + year + section).</p>
            </div>
            ...
            <Card className="p-6 space-y-4">
                ...
                <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs text-white/50">Required columns: department_code, year, section</p>
