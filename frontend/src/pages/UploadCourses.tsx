...
import type { CourseBulkUploadResult } from '../types';

export const UploadCourses: React.FC = () => {
    const [file, setFile] = useState<File | null>(null);
    const [isDownloading, setIsDownloading] = useState(false);
    const [isUploading, setIsUploading] = useState(false);
    const [result, setResult] = useState<CourseBulkUploadResult | null>(null);

    const handleDownloadTemplate = async () => {
        setIsDownloading(true);
        try {
            const blob = await adminApi.downloadCoursesTemplate();
    ...
    const handleUpload = async () => {
        ...
        setIsUploading(true);
        try {
            const data = await adminApi.uploadCoursesCsv(file);
            setResult(data);
    ...
    return (
        <div className="space-y-6 max-w-4xl">
            <div>
                <h1 className="text-2xl font-bold text-white">Upload Courses</h1>
                <p className="text-white/60 text-sm">Admin bulk upload for course records.</p>
            </div>
